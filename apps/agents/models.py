...
from apps.mixins.functions import (generate_coupon_code, 
                                   create_coupon,
                                   send_email_to_user, send_referee_coupon_email, send_referrer_coupon_email)
from apps.mixins.constants import (AGENT_REQUEST_TYPES, 
...
def create_agent_logo_file_path(instance, filename):
    ...
    file_path = f"agents/logo/{instance.pk}_{now:%Y%m%d%H%M%S}_{milliseconds}{extension}"
    return file_path


class Agent(DescriptionAndAddedOnFieldMixin):
    """Agent is a company that list properties to the system"""
    name = models.CharField(verbose_name='agent name', max_length=200, blank=False, null=False)
    motto = models.CharField(verbose_name='agent slogan', max_length=100, null=True, blank=True)
    logo_path = models.ImageField("company logo", upload_to=create_agent_logo_file_path, null=True, blank=True)
    referral_code = models.CharField("agent referral code", max_length=15, unique=True, db_index=True)
    referred_by = models.ForeignKey("self", verbose_name="referrer", on_delete=models.SET_NULL, null=True, blank=True)
...
def agent_post_save(sender, instance, created, **kwargs):
    if created and instance.referred_by:
        ...
        with transaction.atomic():
            available_reward_plan = sys_models.ReferralRewardPlan.objects.filter(expire_on__gt=timezone.now(),
                                                                                 system__name=MYPROPERY_SYSTEM_MODULE_NAME).first()
            if available_reward_plan:
                ...
                referral_tracker_obj, new_created = AgentReferralTracker.objects.update_or_create(
                    defaults = {"agent": referrer_agent,
                ...
                if not new_created:
                    AgentReferralTracker.objects.filter(pk=referral_tracker_obj.id).update(num_of_current_referrals=F("num_of_current_referrals") + 1)
                ...
                system = sys_models.System.objects.filter(name=MYPROPERY_SYSTEM_MODULE_NAME).first() or None

                # CREATE COUPON BASED ON values FIELD OF ListingParameter AND 
                # PERCENTAGE AND FIXED VALUES OF ReferralRewardPlan
                referrer_discount_percentage_value = available_reward_plan.referrer_reward_percentage_value
                referrer_discount_fixed_value = available_reward_plan.referrer_reward_fixed_value
                
                referee_discount_percentage_value = available_reward_plan.referee_reward_percentage_value
                referee_discount_fixed_value = available_reward_plan.referee_reward_fixed_value


                # GET AGENT REFERRAL COUPON LISTING PARAMETER
                listing_param_for_agent_referral_coupon = sys_models.ListingParameter.objects.filter(name=AGENT_REFERRAL_COUPON_PARAM_NAME).first()

                coupon_total_use = 0
                if listing_param_for_agent_referral_coupon and listing_param_for_agent_referral_coupon.is_active:
                    coupon_total_use = int(listing_param_for_agent_referral_coupon.value)
                    will_expire_after_days = listing_param_for_agent_referral_coupon.will_expire_after_days

                #CREATE COUPON FOR REFEREE AGENT
                if referee_discount_percentage_value > 0 or referee_discount_fixed_value > 0:
                    referee_coupon_code = generate_coupon_code()
                    referee_coupon = create_coupon(sys_models.Coupon, 
                                ...
                                use_count = 0,
                                expire_on = datetime.date.today() + datetime.timedelta(days=will_expire_after_days),
                                # listing_parameter = listing_param_for_agent_referral_coupon,
                                system = system)
                    
                    #EMAIL THE CODE TO RFEREE AND REFERRER
                    if referee_coupon:
                        send_referee_coupon_email(referee_coupon, 
                ...
                post_save_referral_tracker_state = AgentReferralTracker.objects.filter(pk=referral_tracker_obj.id)

                if post_save_referral_tracker_state.first().num_of_current_referrals >= \
                    ...
                    referrer_coupon = create_coupon(sys_models.Coupon, 
                    ...
                    if referrer_coupon:
                        ...
                        send_referrer_coupon_email(referrer_coupon, 
                                                   referrer_discount_percentage_value,
                                                     referrer_discount_fixed_value, 
                                                     "zemaedot3@gmail.com")
...
class AgentBranch(DescriptionAndAddedOnFieldMixin):
    ...
    branch_code = models.CharField("agent branch code", max_length=15, unique=True, db_index=True)
    name = models.CharField("agent branch name", max_length=100)
    is_main_branch = models.BooleanField("is this the main branch of the agent", default=False)
    email = models.EmailField("branch email address", max_length=100)
    phone_number = models.CharField(max_length=15)
    address = models.ForeignKey(cmns_models.Address, 
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.UniqueConstraint(fields=["agent", "name"], 
                                    name="unique_branch_name_per_agent_branch"),
    ...
    def clean(self):
        super().clean()
        if self.is_main_branch and \
        self.agent.branches.filter(is_main_branch=True).exclude(id=self.id).exists():
            raise ValidationError("An agent can hav only one main branch!")
...
class AgentAdmin(AddedOnFieldMixin):
    ...
    agent_branch = models.ForeignKey(AgentBranch, 
    ...
    user = models.OneToOneField(settings.AUTH_USER_MODEL, 
                             ...
                             error_messages={"user_not_unique": "A user can create only one agent or \
                                             must be assigned to a single agent as admin"})
    is_superadmin = models.BooleanField(default=False,
                                        help_text="Is this user the super administrator of the branch?")
...
class AgentDiscountTracker(AddedOnFieldMixin, 
                    StartAndExpireOnFieldMixin):
    ...
    discount = models.ForeignKey(sys_models.Discount, 
                                 on_delete=models.CASCADE,
    ...
    used_discounts = models.PositiveIntegerField("how many discounts used so far?", default=0)
    max_discounts = models.PositiveIntegerField("how many total discounts you have?", default=0)
...
class AgentReferralTracker(AddedOnFieldMixin):
    ...
    #                                                       default = 1)
    num_of_current_referrals = models.PositiveIntegerField("current number of referrals?", default = 1)
    referral_reward_plan = models.ForeignKey(sys_models.ReferralRewardPlan, 
    ...
    is_open = models.BooleanField("is tracker open?", 
                                  default=True,
    ...
    def is_rewarded(self):
        return self.num_of_current_referrals == self.referral_reward_plan.number_of_referrals_needed
...
class AgentReferral(AddedOnFieldMixin):
    ...
    referrer_agent = models.ForeignKey(Agent, 
                                       help_text="An agent who referred another agent", 
                                       on_delete=models.CASCADE,
                                       related_name="referrals",
                                       related_query_name="referral")
    referee_agent = models.ForeignKey(Agent, 
                                       help_text="An agent referred by another agent", 
                                       on_delete=models.CASCADE)
    referral_tracker = models.ForeignKey(AgentReferralTracker, 
                                         on_delete=models.CASCADE,
...
class AgentReferralReward(AddedOnFieldMixin):
    ...
    coupon = models.ForeignKey(sys_models.Coupon,
                               on_delete=models.CASCADE,
...
class AgentServiceSubscription(AddedOnFieldMixin,
                               StartAndExpireOnFieldMixin):
    ...
    subscription_currency = models.ForeignKey(sys_models.Currency, on_delete=models.SET_NULL, null=True)
...
class Requester(AddedOnFieldMixin):
    ...
    middle_name = models.CharField("requester middle name", max_length=50, null=True, blank=True)
