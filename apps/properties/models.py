...
from apps.mixins.functions import (property_plan_upload_path,
                                   property_image_path,
                                   property_video_path)
...
class AmenityCategory(DescriptionAndAddedOnFieldMixin):
    ...
    """
    name = models.CharField(verbose_name='amenity category name', max_length=150, unique=True)
    def __str__(self):
        return f"{self.name}"
...
class PropertyCategory(DescriptionAndAddedOnFieldMixin):
    ...
    cat_key = models.CharField(verbose_name="property category key", 
                               max_length=10, 
...
class Property(DescriptionAndAddedOnFieldMixin):
    ...
    custom_prop_id = models.CharField("custom property id", 
                                      max_length=15, 
                                      unique=True)
    property_category= models.ForeignKey(PropertyCategory, 
                                         on_delete=models.CASCADE, 
                                         related_name='category_properties')
    agent_branch = models.ForeignKey(agnt_models.AgentBranch, 
                              on_delete=models.CASCADE, 
                              related_name='branch_properties',
                              related_query_name='branch_property',
                              verbose_name='property agent branch')
    address = models.OneToOneField(cmn_models.Address, 
                                   on_delete=models.SET_NULL, 
                                   related_name="address_properties",
                                   related_query_name="address_property",
                                   verbose_name='property address', 
                                   null=True)
    name = models.CharField("Prperty name or label", max_length=150, blank=True, null=True)
    ...
    tenure = models.CharField(max_length = 100,
                              choices=constants.PROPERTY_TENURE_TYPES)
    tax_band = models.CharField(max_length=50,
                                choices=constants.PROPERTY_TAX_BANDS)
    ...
    @property
    def cat_key(self):
        return f"{self.property_category.cat_key}"
...
class PropertyAmenity(AddedOnFieldMixin):
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(fields=["parent_property","amenity"], name="property_amenity_unique_constraint")
        ]
...
class Apartment(AddedOnFieldMixin,
                PropertyStatusFieldMixin):
    """Apartment"""
    parent_property = models.OneToOneField(Property, 
                                    on_delete=models.CASCADE, 
                                    verbose_name="main property", 
                                    related_name="apartment")
    floors = models.PositiveIntegerField(verbose_name='number of floors in the building', default=0)
    is_multi_unit = models.BooleanField(verbose_name='is multi unit?', default=False)
...
class PropertyPlan(AddedOnFieldMixin,
                   CommonPropertyShallowFieldsMixin):
    """A plan map or blueprint of the proproperty rooms"""
    parent_property = models.ForeignKey(Property, 
                                      on_delete=models.CASCADE,
...
class ApartmentUnit(CommonResidentialFieldsMixin,
                    AddedOnFieldMixin):
    ...
    property_plan = models.OneToOneField(PropertyPlan, 
                                         on_delete=models.SET_NULL, 
                                         null=True, 
                                         blank=True,
    ...
    def __str__(self) -> str:
        return f"Apartment Unit: {self.unit_name_or_number}"
...
class Condominium(CommonResidentialFieldsMixin,
                  AddedOnFieldMixin,
                  CommonPropertyShallowFieldsMixin):
    ...
    def __str__(self):
        return 'Condominium: %s bath rooms and %s bed rooms' % (self.bath_rooms, self.bed_rooms)


class Villa(CommonResidentialFieldsMixin,
            AddedOnFieldMixin,
            CommonPropertyShallowFieldsMixin):
    ...
    def __str__(self):
        return 'Villa: %s bath rooms and %s bed rooms' % (self.bath_rooms, self.bed_rooms)


class Townhouse(CommonResidentialFieldsMixin,
                AddedOnFieldMixin,
                CommonPropertyShallowFieldsMixin):
    ...
    structure = models.CharField("property structure", 
                                 choices=constants.PROPERTY_STRUCTURE,
                                 max_length=100)

    def __str__(self):
        return 'Townhouse: %s bath rooms and %s bed rooms' % (self.bath_rooms, self.bed_rooms)
...
class BuildingType(DescriptionAndAddedOnFieldMixin):
    ...
    type = models.CharField(max_length=100, 
                            choices=constants.BUILDING_TYPES, 
                            unique=True)
    def __str__(self):
        return f"{self.type}"
...
class Sharehouse(AddedOnFieldMixin,
                CommonPropertyShallowFieldsMixin):
    parent_property = models.OneToOneField(Property, 
                                    on_delete=models.CASCADE, 
                                    verbose_name="main property", 
...
class Room(AddedOnFieldMixin,
           CommonResidentialFieldsMixin):
    "Sharehouse rooms"
    sharehouse = models.ForeignKey(Sharehouse, 
                                   on_delete=models.CASCADE,
                                   related_name="rooms",
                                   related_query_name="room")
    has_ensuite_bathroom = models.BooleanField("does the room has ensuite bathroom?", default=False)
    for_gender = models.CharField("required of gender of new flatmate", choices=constants.GENDER, max_length=30)
    for_speaker_of_languages = models.CharField("speaker of language needed", max_length=100, null=True, blank=True)
    flatmate_interests = models.CharField("what interests new flatmate expected to have?", 
                                          max_length=200, 
                                          null=True, 
                                          blank=True)
    ...
    def __str__(self):
        return f"Sharehouse Room: for {self.for_gender}  and speaker of {self.for_speaker_of_languages}"


class CommercialProperty(AddedOnFieldMixin,
                         CommonResidentialFieldsMixin,
                         CommonPropertyShallowFieldsMixin):
...
class OfficeUnit(DescriptionAndAddedOnFieldMixin,
             CommonResidentialFieldsMixin):
    """The office is a commercial property that is used for office purposes"""
    commercial_property = models.ForeignKey(CommercialProperty, 
                                            on_delete=models.CASCADE, 
                                            verbose_name="parent commercial property",
                                            related_name="office_units",
                                            related_query_name="office_unit")
    unit_name_or_number = models.CharField(max_length=100)
    seats = models.PositiveIntegerField("number of seats the office is enough for?", default=1)
    rooms = models.PositiveIntegerField("number of rooms in the office?", default=1)
    property_plan = models.OneToOneField(PropertyPlan, on_delete=models.SET_NULL, null=True, blank=True)
    bath_rooms = None
    bed_rooms = None
    status =None
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(fields=["commercial_property", "unit_name_or_number"], 
...
class OfficeUnitAmenity(AddedOnFieldMixin):
    ...
    other_commercial_property_unit = models.ForeignKey(OfficeUnit, on_delete=models.CASCADE)
    amenity = models.ForeignKey(Amenity, on_delete=models.CASCADE)


class OtherCommercialPropertyUnit(DescriptionAndAddedOnFieldMixin,
                                  CommonResidentialFieldsMixin):
    ...
    property_plan = models.OneToOneField(PropertyPlan, on_delete=models.SET_NULL, null=True, blank=True)
    amenities = models.ManyToManyField(Amenity, through="OtherCommercialPropertyUnitAmenity")
    bath_rooms = None
    bed_rooms = None
    status =None
    is_furnished = None
...
class OtherCommercialPropertyUnitAmenity(AddedOnFieldMixin):
    ...
    other_commercial_property_unit = models.ForeignKey(OtherCommercialPropertyUnit,
                                                       on_delete=models.CASCADE)
    amenity = models.ForeignKey(Amenity, on_delete=models.CASCADE)


"""Hall is a wide room used for meetings and various ceremonies"""
class Venue(AddedOnFieldMixin,
            CommonPropertyShallowFieldsMixin):
    parent_property = models.OneToOneField(Property, 
                                    on_delete=models.CASCADE, 
                                    verbose_name="main property", 
                                    related_name="venue")
    seat_capacity = models.PositiveIntegerField('how many seats the venue has?', default=5)
    total_capacity = models.PositiveIntegerField('total capacity including standing', default=5)
...
class LandType(DescriptionAndAddedOnFieldMixin):
    ...
    def __str__(self) -> str:
        return f"Land Type: {self.name}"


class Land(AddedOnFieldMixin,
           CommonPropertyShallowFieldsMixin):
    """Land properties"""
    parent_property = models.OneToOneField(Property, 
                                    on_delete=models.CASCADE, 
...
class PropertyImage(AddedOnFieldMixin):
    ...
    file_ext = models.CharField("file extension", max_length=10, null=True, blank=True)
    label = models.ForeignKey(PropertyImageLabel, 
...
class PropertyKeyFeature(DescriptionAndAddedOnFieldMixin):
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(fields=["property", "name"],
...
class ListingPriceByPropertyCategory(DescriptionAndAddedOnFieldMixin,
                                     ExpireOnFieldMixin):
    ...
    price_percentage = models.DecimalField(help_text="Percentage value of the property price that the agent should pay for listing",
                                           decimal_places=5,
                                           max_digits=10)
    price_fixed = models.DecimalField(help_text="Fixed value that the agent should pay for listing",
                                           decimal_places=5,
                                           max_digits=10)
    price_lower_bound = models.DecimalField(help_text="minimum listing price",
                                           decimal_places=5,
                                           max_digits=10)
    price_upper_bound = models.DecimalField(help_text="maximum listing price",
                                           decimal_places=5,
                                           max_digits=10)
    currency = models.ForeignKey(sys_models.Currency, on_delete=models.SET_NULL, blank=True, null=True)
...
class ListingPriceByPropertyCategoryHistory(DescriptionAndAddedOnFieldMixin,
                                            ExpireOnFieldMixin):
    """Every category may have a different listing price set by administrators"""
    property_category = models.ForeignKey(PropertyCategory, 
