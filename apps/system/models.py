...
class ListingParameter(DescriptionAndAddedOnFieldMixin):
    ...
    name = models.CharField(
        ...
        max_length=200,
        unique=True,
        choices=constants.LISTING_PARAMETER_NAMES,
    )
    ...
    # will_expire_after_days = models.PositiveIntegerField(
    #     "How many days will conf stay valid", default=1
    # )
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=("system", "name"),
                name="listing_parameter_unique_together_constraint",
            )
        ]
...
class SystemParameter(DescriptionAndAddedOnFieldMixin):
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=("system", "name"),
                name="system_parameter_unique_together_constraint",
            )
        ]
...
class Currency(AddedOnFieldMixin):
    ...
    iso_4217_alpha_code = models.CharField(
        "alphabetic code of the currency", max_length=3, unique=True, default="ETB"
    )
    iso_4217_num_code = models.CharField(
        "numeric code of the currency", max_length=3, unique=True, default="230"
    )
    is_default = models.BooleanField(default=False)
    exchange_rate = models.DecimalField(
        decimal_places=5, max_digits=10, default=0.000, blank=True
    )
...
class PaymentMethodDiscount(
    StartAndExpireOnFieldMixin, DescriptionAndAddedOnFieldMixin
):
...
class Coupon(StartAndExpireOnFieldMixin, AddedOnFieldMixin):
    """
    ...
    code = models.CharField(
        "coupon code", unique=True, max_length=20, null=False, blank=True
    )
    discount_percentage_value = models.DecimalField(
        decimal_places=2, max_digits=10, default=0.00
    )
    ...
    total_use = models.PositiveIntegerField(
        "total uses", default=1, help_text="How many times can be redeemed"
    )
    use_count = models.PositiveIntegerField(
        "use counts", default=0, help_text="How many times redeemed so far"
    )
...
class Voucher(StartAndExpireOnFieldMixin, AddedOnFieldMixin):
    """
    ...
    code = models.CharField(
        "voucher code", unique=True, max_length=20, null=False, blank=True
    )
    initial_value = models.DecimalField(
        "initial voucher value", default=0.00, decimal_places=5, max_digits=12
    )
    current_value = models.DecimalField(
        "current voucher value", default=0.00, decimal_places=5, max_digits=12
    )
    base_currency = models.ForeignKey(
        Currency,
        on_delete=models.CASCADE,
...
class SystemAsset(DescriptionAndAddedOnFieldMixin):
    ...
    name = models.CharField(
        "system asset name", max_length=100, blank=False, null=False
    )
...
class ReferralRewardPlan(DescriptionAndAddedOnFieldMixin, StartAndExpireOnFieldMixin):
    system = models.ForeignKey(
        System,
        verbose_name="System Module",
        on_delete=models.CASCADE,
    ...
    referrer_reward_percentage_value = models.DecimalField(
        decimal_places=2, max_digits=10, default=0.00
    )
    referrer_reward_fixed_value = models.DecimalField(
        decimal_places=2, max_digits=10, default=0.00
    )
    referee_reward_percentage_value = models.DecimalField(
        decimal_places=2, max_digits=10, default=0.00
    )
    referee_reward_fixed_value = models.DecimalField(
        decimal_places=2, max_digits=10, default=0.00
    )
    number_of_referrals_needed = models.PositiveIntegerField(
        default=1,
        blank=True,
    ...
    def save(self, *args, **kwargs):
        ...
        existing_unexpired_reward_plan = (
            ReferralRewardPlan.objects.filter(
                system=self.system, expire_on__gt=timezone.now()
            )
            .exclude(id=self.id)
            .first()
        )
        ...
        if (existing_unexpired_reward_plan and not self.id) or (
            existing_unexpired_reward_plan
            and self.id
            and self.expire_on > timezone.now()
        ):
